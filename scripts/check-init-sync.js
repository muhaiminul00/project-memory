...
// Verifies commands/memory-init.md's embedded CLAUDE.md starter block is
...
const cmdPath = path.join(repoRoot, 'commands', 'memory-init.md');
const marker = '<!-- project-memory-plugin:v1 -->';

const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), 'project-memory-sync-check-'));
...
try {
  ...
  if (!fenceMatch) {
    console.error('FAIL: could not find the fenced starter block in commands/memory-init.md');
    process.exit(1);
  }
  ...
  if (actual === embedded) {
    console.log('OK: commands/memory-init.md\'s starter block matches the real hook output.');
  } else {
    console.error('FAIL: commands/memory-init.md has drifted from hooks/session-start.js\'s seedClaudeMd() output.');
    console.error('Re-copy the block from a fresh run (see the ground-truth method this script uses) into commands/memory-init.md.');
    process.exit(1);
  }
} finally {
